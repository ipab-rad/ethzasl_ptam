...
class Tracker {
 public:
  ...
  ComputeGrid();              // Computes the reference grid;

 protected:
  CVD::Image<TooN::Vector<2> > mProjVertices;
  KeyFrame::Ptr
  mCurrentKF;            // The current working frame as a keyframe struct
  ...
  KeyFrame::Ptr
  mPreviousFrameKF;      // Used by trail tracking to check married matches
