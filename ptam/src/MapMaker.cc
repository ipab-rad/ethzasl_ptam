    ...
    if (mMap.vpPoints[i]->bBad) {
      ...
      for (unsigned int j = 0; j < mMap.vpKeyFrames.size(); j++) {
        ...
        if (k.mMeasurements.count(p)) {
          ...
          for (unsigned int ibest = 0; ibest < k.iBestPointsCount;
               ibest++) { //check all best points
            //slynen copy this
            if (k.apCurrentBestPoints[ibest] ==
                p) { //is the point out of range or to be deleted one of our bestPoints?
              k.apCurrentBestPoints[ibest].reset();
              int mindist = 9999;
              int tempdist = 0;
              Vector<2> ptlev0 = LevelZeroPos(p->irCenter,
                                              p->nSourceLevel); //to level zero coords
              for (int iP = k.vpPoints.size() - 1; iP >= 0;
                   iP--) { //search a nearby point as new bestPoint
                Vector<2> pt2lev0 = LevelZeroPos(k.vpPoints[iP]->irCenter,
                                                 k.vpPoints[iP]->nSourceLevel);
                tempdist = abs(ptlev0[0] - pt2lev0[0]) + abs(ptlev0[1] -
                                                             pt2lev0[1]);   //calc simple dist
                if (tempdist < mindist &&
                    ! k.vpPoints[iP]->bBad) {         //if closer and not bad
                  mindist = tempdist;
...
//Weiss{
  ...
  if (dTransMagn == 0) {
    mMessageForUser << "  Estimated zero baseline from stereo pair, try again." <<
                    endl;
    return false;
  }
  ...
  for (unsigned int i = 0; i < vMatches.size(); i++) {
    ...
    p->v3OneDownFromCenter_NC = unproject(mCamera.UnProject(p->irCenter + ImageRef(
                                                              0, 1)));
    p->v3OneRightFromCenter_NC = unproject(mCamera.UnProject(p->irCenter + ImageRef(
                                                               1, 0)));
...
//Weiss{
  ...
  if (((pkFirst->dSceneDepthSigma + pkSecond->dSceneDepthSigma) / 2.0 > 0.5) &&
      pParams.CheckInitMapVar) {
    ROS_WARN_STREAM("Initial map rejected because of too large point variance. Point sigma: "
                    << ((pkFirst->dSceneDepthSigma + pkSecond->dSceneDepthSigma) / 2.0));
    return false;
  }
  ...
  mMessageForUser << "  MapMaker: made initial map with " << mMap.vpPoints.size()
                  << " points." << endl;
...
bool MapMaker::AddSomeMapPoints(int nLevel) {
  ...
  bool addedsome = false;
  //}
  KeyFrame::Ptr kSrc = (mMap.vpKeyFrames[mMap.vpKeyFrames.size() -
                                         1]); // The new keyframe
...
void MapMaker::AddKeyFrameFromTopOfQueue() {
  ...
  if (pPars.MaxKF > 1) { // MaxKF<2 means keep all KFs
    while (mMap.vpKeyFrames.size() > (unsigned int)pPars.MaxKF) {
      ...
      for (vector<KeyFrame::Ptr>::iterator it = mMap.vpKeyFrames.begin();
  ...
  bool addedsome = false;
  addedsome |= AddSomeMapPoints(
                 3);       // .. and add more map points by epipolar search.
...
bool MapMaker::AddPointEpipolar(KeyFrame::Ptr kSrc,
  ...
  Vector<3> v3Ray_SC = unproject(mCamera.UnProject(
                                   v2RootPos)); // direction vector from current camera to the feature? [SW]
  normalize(v3Ray_SC);
  Vector<3> v3LineDirn_TC = kTarget->se3CfromW.get_rotation() *
                            (kSrc->se3CfromW.get_rotation().inverse() *
                             v3Ray_SC); // direction vector from target KF camer to the feature? [SW]
  ...
  //  double dStartDepth = max(mdWiggleScale, dMean - dSigma); // estimated minimal distance of the feature in the current KF? [SW]
  double dStartDepth = max(0.0,
                           dMean - dSigma); // estimated minimal distance of the feature in the current KF? [SW]
  double dEndDepth = dMean +
                     dSigma; // estimated maximal distance of the feature in the current KF? [SW]
  //}
  Vector<3> v3CamCenter_TC = kTarget->se3CfromW *
                             kSrc->se3CfromW.inverse().get_translation(); // The camera of the current camera seen from the target KF? [SW]
  Vector<3> v3RayStart_TC = v3CamCenter_TC + dStartDepth *
                            v3LineDirn_TC;                           // start point on the 3D epi-line expressed in the target KF? [SW]
  Vector<3> v3RayEnd_TC = v3CamCenter_TC + dEndDepth *
                          v3LineDirn_TC;                               // end point on the 3d epi-line expressed in the target KF? [SW]
  ...
  double dMinLen = min(v2AlongProjectedLine * v2A,
                       v2AlongProjectedLine * v2B) - 0.05;
  double dMaxLen = max(v2AlongProjectedLine * v2A,
                       v2AlongProjectedLine * v2B) + 0.05;
  ...
  pNew->v3OneRightFromCenter_NC = unproject(mCamera.UnProject(v2RootPos + vec(
                                                                ImageRef(nLevelScale, 0))));
  pNew->v3OneDownFromCenter_NC  = unproject(mCamera.UnProject(v2RootPos + vec(
                                                                ImageRef(0, nLevelScale))));
...
bool MapMaker::NeedNewKeyFrame(KeyFrame::Ptr kCurrent) {
  ...
  Vector<3> veccam2;
  Vector<2> cam2, cam1;
  for (std::map<MapPoint::Ptr, Measurement>::iterator it =
         kCurrent->mMeasurements.begin(); it != kCurrent->mMeasurements.end(); it++) {
    veccam2 = pClosest->se3CfromW * it->first->v3WorldPos; // rotate in closest KF
    veccam2 = (kCurrent->se3CfromW * pClosest->se3CfromW.inverse()).get_rotation() *
              veccam2; // derot in current frame
    cam2 = mCamera.Project(makeVector(veccam2[0] / veccam2[2],
                                      veccam2[1] / veccam2[2]));
    cam1 = it->second.v2RootPos;
    medianpixdist.push_back(sqrt((cam2[0] - cam1[0]) * (cam2[0] - cam1[0]) +
                                 (cam2[1] - cam1[1]) * (cam2[1] - cam1[1])));
  }
  ...
  std::nth_element(first, middle,
                   last); // can specify comparator as optional 4th arg
  double mediandist = *middle;

  dDist *= (1.0 / kCurrent->dSceneDepthMean);


  Matrix<> mat1 = kCurrent->se3CfromW.get_rotation().get_matrix();
  Matrix<> mat2 = pClosest->se3CfromW.get_rotation().get_matrix();
  ...
  double dist = sqrt(sum) * 57.2957795;
...
void MapMaker::BundleAdjust(set<KeyFrame::Ptr> sAdjustSet,
                            set<KeyFrame::Ptr> sFixedSet, set<MapPoint::Ptr> sMapPoints, bool bRecent) {
  ...
  for (set<KeyFrame::Ptr>::iterator it = sAdjustSet.begin();
  ...
  for (set<KeyFrame::Ptr>::iterator it = sFixedSet.begin(); it != sFixedSet.end();
       it++) {
  ...
  for (set<MapPoint::Ptr>::iterator it = sMapPoints.begin();
  ...
  for (unsigned int i = 0; i < vOutliers_PC_pair.size(); i++) {
    ...
    if (pp->pMMData->GoodMeasCount() <= 2 ||
        m.Source ==
        Measurement::SRC_ROOT)  // Is the original source kf considered an outlier? That's bad.
      pp->bBad = true;
    else {
...
bool MapMaker::ReFind_Common(KeyFrame::Ptr k, MapPoint::Ptr p) {
  ...
  if (v2ImPlane * v2ImPlane > mCamera.LargestRadiusInImage() *
      mCamera.LargestRadiusInImage()) {
    p->pMMData->sNeverRetryKFs.insert(k);
    return false;
  }
  ...
  bool bFound = Finder.FindPatchCoarse(ir(v2Image), k,
                                       4);  // Very tight search radius!
  if (!bFound) {
    p->pMMData->sNeverRetryKFs.insert(k);
    return false;
  }
...
SE3<> MapMaker::CalcPlaneAligner() {
  ...
  if (nPoints < 10) {
    mMessageForUser << "  MapMaker: CalcPlane: too few points to calc plane." <<
                    endl;
    return SE3<>();
  };
  //Weiss{
...
void MapMaker::GUICommandHandler(string sCommand,
  ...
  mMessageForUser << "! MapMaker::GUICommandHandler: unhandled command " <<
                  sCommand << endl;
  exit(1);
};
