    ...
    if (mnLostFrames < 3) { // .. but only if we're not lost!
...

      {
        ...
        for (int i = 0; i < LEVELS;
             i++) mMessageForUser << " " << manMeasFound[i] << "/" << manMeasAttempted[i];
    ...
    if (pPars.AutoInit) {
      if (mnInitialStage == TRAIL_TRACKING_NOT_STARTED)
        ...
        for (list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end(); ++i) {
          ...
          z1vec = mCamera.Project(makeVector(dirvec[0] / dirvec[2],
                                             dirvec[1] / dirvec[2]));
          mediandistvec.push_back(sqrt((z1vec[0] - firstvec[0]) *
                                       (z1vec[0] - firstvec[0]) + (z1vec[1] - firstvec[1]) * (z1vec[1] -
                                           firstvec[1])));
          ++k;
        }
        delete pSBIsecond;
...
void Tracker::GUICommandHandler(string sCommand,
  ...
  if (sCommand == "KeyPress") {
    if (sParams == "Space") {
      ...
      mAutoreset = false;
      ROS_WARN_STREAM("Forcing map reset because of user input! Autoreset set to " <<
                      mAutoreset);
      Reset();
...
void Tracker::TrackForInitialMap() {
  ...
  if (mnInitialStage == TRAIL_TRACKING_STARTED) {
    ...
    if (mbUserPressedSpacebar) {
      ...
      bool initret = false;
      initret = mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches,
      ...
      if (initret) {
        if (mAutoreset) {
          ...
          for (unsigned int ip = 0; ip < mMap.vpPoints.size(); ip++) {
            ...
            medianvec1.push_back((mMap.vpKeyFrames[0]->se3CfromW *
                                  mMap.vpPoints[ip]->v3WorldPos)[2]);
            //            else //if(mMap.vpPoints[ip]->pPatchSourceKF==mMap.vpKeyFrames[1])
            medianvec2.push_back((mMap.vpKeyFrames[1]->se3CfromW *
                                  mMap.vpPoints[ip]->v3WorldPos)[2]);
          }
          ...
          double scale = mOldKF->dSceneDepthMedian /
                         mMap.vpKeyFrames[0]->dSceneDepthMedian;
...
void Tracker::TrailTracking_Start() {
  ...
  *mFirstKF = *mCurrentKF; //copy data
  ...
  int level = PtamParameters::fixparams().InitLevel;
  for (unsigned int i = 0; i < mCurrentKF->aLevels[level].vCandidates.size();
    ...
    if (!mCurrentKF->aLevels[level].im.in_image_with_border(c.irLevelPos,
...
void Tracker::TrackMap() {
  ...
  if (mTrackingQuality == GOOD) { //if tracking quality is not good, we use all KF
    for (unsigned int k = 0; k < mMap.vpKeyFrames.size(); k++) {
      //project best points
      int foundKFPoints = 0;
      if (!mMap.vpKeyFrames.at(k)->vpPoints.size())
        continue; //if this keyframe doesn't yet contain any points
  ...
  for (unsigned int k = 0; k < vpPVKeyFrames.size();
       k++) { //for all possibly visible keyframes
    for (unsigned int i = 0; i < vpPVKeyFrames.at(k)->vpPoints.size();
         i++) { // For all points in the visible keyframes..
      MapPoint::Ptr p = (vpPVKeyFrames.at(k)->vpPoints.at(i));
      if (p->bAlreadyProjected)
        continue; //check whether we already projected that point from another KF
      p->bAlreadyProjected = true;
      if (!p->pTData)
        p->pTData = new TrackerData(
          p);// Ensure that this map point has an associated TrackerData struct.
      TrackerData& TData = *p->pTData;
      TData.Project(mse3CamFromWorld,
                    mCamera);// Project according to current view, and if it's not in the image, skip.
      if (!TData.bInImage)
        continue;

      TData.GetDerivsUnsafe(
        mCamera);// Calculate camera projection derivatives of this point.

      // And check what the PatchFinder (included in TrackerData) makes of the mappoint in this view..

      TData.nSearchLevel = TData.Finder.CalcSearchLevelAndWarpMatrix(TData.Point,
                                                                     mse3CamFromWorld, TData.m2CamDerivs);
  ...
  // Finally, find the mean scene depth from tracked features
  {
    ...
    if (nNum > 20) {
      ...
      mCurrentKF->dSceneDepthSigma = sqrt((dSumSq / nNum) -
...
int Tracker::SearchForPoints(vector<TrackerData*>& vTD, int nRange,
  ...
  for (unsigned int i = 0; i < vTD.size(); i++) { // for each point..
    ...
    if (nSubPixIts > 0) {
      ...
      bool bSubPixConverges = Finder.IterateSubPixToConvergence(*mCurrentKF,
                                                                nSubPixIts);
      if (!bSubPixConverges) {
...
Vector<6> Tracker::CalcPoseUpdate(vector<TrackerData*> vTD,
  ...
  for (unsigned int f = 0; f < vTD.size(); f++) {
    ...
    vdErrorSquared.push_back(static_cast<double>(TD.v2Error_CovScaled *
                                                 TD.v2Error_CovScaled));
  };
...
void Tracker::AssessTrackingQuality() {
  ...
  if (mTrackingQuality == BAD) {
    ...
    if (pPars.AutoInit & mMap.IsGood()) {
      mAutoreset = true;
      ROS_WARN_STREAM("Forcing map reset because of bad tracking! Autoreset set to "
                      << mAutoreset);
    }
    //}
    mnLostFrames++;
