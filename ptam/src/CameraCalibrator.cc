...
int main(int argc, char** argv) {
  ...
  ROS_INFO("starting CameraCalibrator with node name %s",
  ...
  GV3::get<Vector<NUMTRACKERCAMPARAMETERS> >("Camera.Parameters",
                                             ATANCamera::mvDefaultParams, SILENT);

  try {
    std::cout << "Gui is " << (PtamParameters::fixparams().gui ? "on" : "off") <<
              std::endl; //make the singleton instantiate
    CameraCalibrator c;
    c.Run();
...
CameraCalibrator::CameraCalibrator() :
  mCamera("Camera"), mbDone(false), mCurrentImage(CVD::ImageRef(752, 480)),
  mDoOptimize(false), mNewImage(false) {
  ros::NodeHandle nh;
...
void CameraCalibrator::Run() {
  ...
  while (!mbDone && ros::ok()) {
    ...
    if (!*mgvnOptimizing) {
      ...
      mDoOptimize =
        true; // set this so that optimization begins when "optimize" is pressed)
    ...
    if (!*mgvnOptimizing) {
      ...
      ost << "(That would be a pixel aspect ratio of " << mCamera.PixelAspectRatio()
          << ")" << endl;
...
void CameraCalibrator::GUICommandHandler(string sCommand,
...
void CameraCalibrator::OptimizeOneStep() {
  ...
  for (int n = 0; n < nViews; n++) {
    ...
    for (unsigned int i = 0; i < vEAJ.size(); i++) {
      ...
      mJTJ.slice(nMotionBase, nMotionBase, 6, 6) =
        mJTJ.slice(nMotionBase, nMotionBase, 6,
                   6) + EAJ.m26PoseJac.T() * EAJ.m26PoseJac;
      mJTJ.slice(nCamParamBase, nCamParamBase, NUMTRACKERCAMPARAMETERS,
                 NUMTRACKERCAMPARAMETERS) =
                   mJTJ.slice(nCamParamBase, nCamParamBase, NUMTRACKERCAMPARAMETERS,
                              NUMTRACKERCAMPARAMETERS) + EAJ.m2NCameraJac.T() * EAJ.m2NCameraJac;
      mJTJ.slice(nMotionBase, nCamParamBase, 6, NUMTRACKERCAMPARAMETERS) =
        mJTJ.slice(nMotionBase, nCamParamBase, 6,
                   NUMTRACKERCAMPARAMETERS) + EAJ.m26PoseJac.T() * EAJ.m2NCameraJac;
      ...
      vJTe.slice(nCamParamBase, NUMTRACKERCAMPARAMETERS) =
        vJTe.slice(nCamParamBase,
                   NUMTRACKERCAMPARAMETERS) + EAJ.m2NCameraJac.T() * EAJ.v2Error;
  ...
  if (nTotalMeas == 0) {
    ROS_WARN_THROTTLE(2,
                      "No new measurements, this can happen for wide angle cameras when \"Camera.Project()\" gets invalid");
    return;
  }
  ...
  for (int n = 0; n < nViews; n++)
    mvCalibImgs[n].mse3CamFromWorld = SE3<>::exp(vUpdate.slice(n * 6,
                                                               6)) * mvCalibImgs[n].mse3CamFromWorld;
